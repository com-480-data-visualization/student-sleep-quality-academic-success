...
We split the work into three roles for Milestone~2. \textbf{Ali} built the website skeleton and the chart scaffolds in D3 v7; \textbf{Elio} designed the layout grid and the colour system; \textbf{Hana} wrote the Milestone~2 report and prepared the survey-spec reference table we would later use for data validation. We submitted the prototype with the heatmap for RQ1 implemented, the bar chart and parallel-coordinates scaffolds in place, and \emph{simulated data} that approximated what we believed the real dataset would look like. That last decision — simulating instead of loading — is what set us up for the pivot in Milestone~3.

% =========================  S2 — M2 mistakes  ============================
\section{Milestone~2: sketches and what we got wrong}
\label{sec:m2}

We submitted five hand-drawn sketches with the Milestone~2 report (Figures~1--5 of that document): the sleep-quality $\times$ performance heatmap, the duration $\times$ performance grouped bar, the lifestyle parallel coordinates, the Sankey flow as a stretch goal, and the radar comparison as a second stretch goal. The grader feedback was encouraging, but two issues surfaced once we plugged in the real CSV at the start of Milestone~3.

\begin{tcolorbox}[title={Two mistakes baked into the prototype}]
\textbf{1. Label drift.} Our heatmap legend used \emph{Poor / Fair / Average / Good / Excellent} for sleep quality. The real survey labels are \emph{Very poor / Poor / Average / Good / Very good}. We had also assumed a \code{5--6 hours} duration bucket that simply does not exist in the dataset — the survey jumps from \code{4--5 hours} to \code{6--7 hours}.

\textbf{2. Simulated data hid the real story.} The placeholder distribution we generated for the grouped bar deliberately encoded a strong positive correlation between sleep duration and performance. When we loaded the real data we found the opposite — proportions barely shifted across duration buckets.
\end{tcolorbox}
...
We took that strict parser as the contract for the rest of the project. Every chart receives normalised \code{\{raw, ord\}} pairs for ordinal fields, so sorting and scaling use the integer ord while tooltips and labels show the original survey wording. We promised ourselves we would not invent values again.
...
Figures referenced throughout this document: the five sketches we submitted with Milestone~2 (heatmap, grouped bar, parallel coords, Sankey, radar) — see the Milestone~2 report — plus screenshots of the final site (hero, scrollytelling step~3, heatmap residual mode, Sankey full-width view, radar comparison) included in the project's GitHub repository.
